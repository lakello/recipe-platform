...
Сеть, IAM-аккаунты и Yandex Managed Kubernetes развёрнуты через Terraform. Control plane работает, node groups `system` и `app` подключены; Kubernetes-ноды проверены в статусе `Ready`. Остальные модули dev-окружения этим этапом не разворачивались.
