...
В процессе. DevOps-этап начат.

Выполнено:

- Yandex Cloud настроен: создан cloud, folder, billing account.
- Установлен и авторизован Yandex Cloud CLI.
- Создан сервисный аккаунт `recipe-service` с ролью `editor` на folder.
- Создан static access key для Terraform backend.
- Создана структура `envs/dev/` с `terraform.tfvars.example`.

Следующий шаг по приоритету:

1. Remote state bucket (Object Storage).
2. Yandex Cloud provider configuration.
3. Network module.
4. Security groups.
...
9. Bastion/self-hosted runner.
10. DNS.
11. Lockbox.
12. Terraform CI pipeline.
