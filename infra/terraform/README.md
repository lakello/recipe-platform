...
infra/terraform/
  ...
  modules/
    ...
    kubernetes/     # Yandex Managed Kubernetes cluster и node groups
...
Базовая структура готова. `terraform init` работает.

Выполнено:

- Yandex Cloud настроен: cloud, folder, billing account, CLI.
- Создан сервисный аккаунт `recipe-service` с ролью `editor`.
- Настроен S3 remote backend (Yandex Object Storage, bucket `recipe-platform-tfstate`).
- Настроен Yandex Cloud provider (`providers.tf`) с аутентификацией через `authorized_key.json`.
- Добавлены `variables.tf`, `outputs.tf`, `terraform.tfvars.example`, `backend.hcl.example`.
- Добавлен `Makefile` с полным набором команд для работы с dev-окружением.
- Созданы заготовки всех 8 модулей (`network`, `kubernetes`, `postgres`, `redis`, `object-storage`, `compute`, `dns`, `iam`).
- Созданы заготовки окружений `staging` и `prod`.

Следующий шаг: реализация модулей, начиная с `network`.
