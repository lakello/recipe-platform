...
- отдельный service account имеет доступ только к state bucket;
- доступ людям и CI выдаётся только на время необходимой операции;
- state не хранится в Git;
- секреты не должны попадать в outputs и CI-логи.

Terraform state содержит исходные значения ресурсов, включая данные, помеченные
как `sensitive`. Поэтому state и сохранённые plan-файлы считаются секретами: их
нельзя прикладывать к задачам, публиковать в логах или передавать через Git.
Статические ключи backend хранятся только вне репозитория и регулярно
ротируются.

Текущий `backend.tf` (dev):
...
Credentials (`access_key`, `secret_key`) передаются отдельно через `backend.hcl` (не коммитится):
...
В окружении `dev` доступен `Makefile`:
...
make apply    # terraform apply (использует сохранённый .tfplan)
...
Для `staging` и `prod` Makefile ещё не добавлен — пока через прямые команды:
...
Базовая структура, remote state и сетевой модуль готовы.

Выполнено:

- Yandex Cloud настроен: cloud, folder, billing account, CLI.
- Создан сервисный аккаунт `recipe-service` с ролью `editor`.
- Создан bucket `recipe-platform-tfstate` с versioning и закрытым публичным доступом.
- Настроен S3 remote backend для `dev`, `staging`, `prod` (единый bucket, разные ключи).
- Настроен Yandex Cloud provider с аутентификацией через `authorized_key.json`.
...
Следующий шаг: реализация модуля `iam`.
