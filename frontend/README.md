...
- Tailwind CSS v3
- React Hook Form + Zod
- @dnd-kit (сортировка шагов)
- ESLint + typescript-eslint
- Docker (multi-stage: Node 22 → Nginx 1.25)
- Nginx (SPA fallback, proxy `/api/` на backend, cache headers)
...
frontend/
  src/
    ...
    widgets/
      admin-layout/     — sidebar с ролевой навигацией
    features/           — функции с api/, hooks/, ui/
      auth/
      profile/
      recipes/
      categories/
...
/login          — публичная
/register       — публичная
/recipes        — публичная (список рецептов)
/recipes/:id    — публичная (страница рецепта)
/recipes/new    — 🔒
/recipes/:id/edit — 🔒
/recipes/drafts — 🔒
/profile        — 🔒
/favorites      — 🔒
/feed           — 🔒 (лента от подписок)
/users/:id      — публичная (профиль пользователя)
/users/:id/followers — публичная
/users/:id/following — публичная
/search         — публичная
/meal-plan      — 🔒
/shopping-list  — 🔒
/notifications  — 🔒
/admin/users    — 🔒 admin
/admin/recipes  — 🔒 moderator
...
1. Node 22 — сборка React-приложения через Vite.
2. Nginx 1.25 — раздача статики, SPA fallback, proxy `/api/` на backend, cache headers.
...
Второй этап разработки завершён. Frontend полностью реализован.

Реализовано:

1. ~~Базовая структура Vite + React + TypeScript (FSD).~~ ✓
2. ~~Routing, ProtectedRoute, AdminRoute.~~ ✓
3. ~~API client с auto-refresh на 401.~~ ✓
...
13. ~~Поиск рецептов (OpenSearch).~~ ✓
14. ~~Meal plan (недельный, копирование).~~ ✓
15. ~~Shopping list (Celery async + polling, группировка).~~ ✓
