...
backend/
  app/
    api/              — HTTP endpoints (роутеры по функциям, без версионирования)
    core/             — конфигурация, security, логирование, middleware, OpenSearch, S3
    db/               — SQLAlchemy session, base model
    models/           — SQLAlchemy-модели
    repositories/     — слой доступа к данным (SQL-запросы)
    schemas/          — Pydantic-схемы запросов и ответов
    services/         — бизнес-логика
    tasks/            — Celery-задачи (email, shopping_list, thumbnails)
    celery_app.py     — настройка Celery
    main.py           — точка входа FastAPI
  alembic/
    versions/         — файлы миграций БД
  scripts/
    check_alembic_heads.py   — проверка единственного Alembic head (pre-commit хук)
    reindex_opensearch.py    — переиндексация всех рецептов в OpenSearch
    seed_admin.py            — создание суперадмина из env-переменных
  tests/              — unit-тесты сервисов (плоская структура)
  Dockerfile
  .dockerignore
  pyproject.toml
  alembic.ini
...
- `/api/ingredient-categories` — категории ингредиентов
- `/api/ingredients` — справочник ингредиентов, шаги
- `/api/likes` — лайки и избранное
- `/api/comments` — комментарии и ответы
- `/api/follows` — подписки на авторов
...
- `GET /ready` — readiness probe (проверяет PostgreSQL)

### `app/core/`

Общая конфигурация приложения:

- `config.py` — класс `Settings` через pydantic-settings;
- `security.py` — создание/валидация JWT, bcrypt хэширование;
- `logging.py` — JSON-логирование в stdout, request_id;
- `middleware.py` — CorrelationId middleware;
- `exceptions.py` — обработчики HTTP-ошибок и 422/500;
- `storage.py` — boto3-клиент для MinIO/S3;
- `opensearch.py` — AsyncOpenSearch клиент и инициализация индекса.
...
- `email.py` — отправка email-уведомлений через SMTP; retry до 5 раз с exponential backoff;
- `shopping_list.py` — асинхронная генерация списка покупок из плана питания;
- `thumbnails.py` — генерация thumbnail (заглушка, будет реализована позже).
...
Тесты находятся в директории `tests/` (плоская структура, без подпапок).

Тестируются сервисы: auth, user, recipe, category, ingredient, like, comment, follow, meal_plan, shopping_list, search, admin.
...
Второй этап разработки завершён. Backend полностью реализован.
...
4. ~~Alembic-миграции.~~ ✓
5. ~~Auth (регистрация, логин, JWT, refresh-ротация).~~ ✓
6. ~~Users и profiles.~~ ✓
7. ~~Recipes CRUD.~~ ✓
...
11. ~~Likes, favorites.~~ ✓
12. ~~Comments (с ответами и модерацией).~~ ✓
13. ~~Public user profiles.~~ ✓
14. ~~Follows и feed.~~ ✓
15. ~~Search (OpenSearch).~~ ✓
16. ~~Meal plans.~~ ✓
17. ~~Shopping lists (async Celery + polling).~~ ✓
18. ~~OAuth Google и Яндекс.~~ ✓
19. ~~Moderation и admin.~~ ✓
20. ~~Уведомления и email-доставка (Celery, SMTP, Mailhog).~~ ✓
